...
const db = new sqlite3.Database('./bepviet.db', (err) => {
    if (err) {
...
db.serialize(() => {
    ...
    db.run(`CREATE TABLE IF NOT EXISTS Recipes (
        ...
        tip TEXT,
        image TEXT,
        is_featured INTEGER DEFAULT 0, -- Tích hợp luôn cột thịnh hành vào lúc tạo bảng
        FOREIGN KEY (category_id) REFERENCES Categories(id) ON DELETE SET NULL
    )`);
    
    // Đề phòng trường hợp bảng đã tồn tại từ trước nhưng chưa có cột is_featured (Chạy ngầm)
    db.run("ALTER TABLE Recipes ADD COLUMN is_featured INTEGER DEFAULT 0", (err) => { /* Bỏ qua nếu cột đã tồn tại */ });
    ...
    db.get("SELECT COUNT(*) as count FROM Users", (err, row) => {
        if (row && row.count === 0) {
            ...
            console.log("🔑 Đã khởi tạo tài khoản Admin mặc định (admin / bepviet2026)");
        }
    });
...
// Lấy danh sách toàn bộ món ăn (Đã tối ưu truy vấn N+1)
...
// Lấy thống kê cho trang Admin Dashboard (Đã gộp 2 API bị trùng)
...
app.put('/api/recipes/:id', upload.single('image'), (req, res) => {
    ...
    db.get("SELECT id FROM Categories WHERE name = ?", [category], (err, catRow) => {
        ...
        if (req.file) {
            recipeSql += `, image=?`;
            recipeParams.push(`http://localhost:${PORT}/uploads/${req.file.filename}`);
        }
        recipeSql += ` WHERE id=?`;
...
// Bật/Tắt trạng thái Thịnh hành (Đã gộp API bị trùng)
...
app.delete('/api/recipes/:id', (req, res) => {
    ...
    // Nhờ ON DELETE CASCADE khi khởi tạo bảng, SQLite sẽ tự động dọn dẹp các bảng phụ (Ingredients, Steps)
...
app.listen(PORT, () => {
    console.log(`🚀 Backend quan hệ đã chạy tại: http://localhost:${PORT}`);
    
    // Heartbeat log (Tùy chọn: báo console để biết server còn sống)
    setInterval(() => {
        console.log("💖 Server vẫn đang sống...");
    }, 3600000); // Đổi thành 1 tiếng log 1 lần để đỡ rác terminal thay vì 10 giây
});